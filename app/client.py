...
# Resolve paths relative to this client kit so the application is portable
# across institutions and filesystem layouts.
KIT_ROOT = Path(
    os.environ.get(
...
DEFAULT_DATA_ROOT = KIT_ROOT / "data" / "cifar10"
DEFAULT_TRAIN_IDX_ROOT = KIT_ROOT / "splits"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to local client configuration JSON file.",
    )
    parser.add_argument(
        "--train_idx_root",
        type=str,
        default=None,
        help="Root directory containing the local training index files. "
             "Defaults to the splits directory in this client kit.",
    )
    parser.add_argument(
        "--data_root",
        type=str,
        default=None,
        help="Root directory containing the local CIFAR-10 dataset. "
             "Defaults to the data/cifar10 directory in this client kit.",
    )
    parser.add_argument(
        "--aggregation_epochs",
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.01,
        help="Minimum learning rate as a factor of the initial learning rate for cosine annealing scheduler. Default is 0.01 (or 1 percent).",
    )
    ...
    if args.config is None:
        args.config = os.environ.get("CIFAR10_CLIENT_CONFIG")
    ...
    if args.config is not None:
        ...
        if args.train_idx_root is None:
            args.train_idx_root = config.get("train_idx_root")

    if args.data_root is None:
        args.data_root = str(DEFAULT_DATA_ROOT)

    if args.train_idx_root is None:
        args.train_idx_root = str(DEFAULT_TRAIN_IDX_ROOT)
